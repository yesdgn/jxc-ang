...
@Component({
  selector: 'dynamic-form-tree-tree',
  templateUrl: 'df-form-tree-tree.component.html',
  styleUrls: ['./df-form-tree-tree.component.css']
})
export class DFormTreeTreeComponent implements OnChanges {
  @Input() pageConfig;
  ...
  pageInit(): Promise<any> { //单表头
    ...
    return promise.then  <any>(() => {
      ...
      .then((data) => {
        ...
        return this.pageService.readTreeMenu(treeControl.ApiID, treeControl.order, treeControl.colName, treeControl.label);
      })
      .then((data) => {
